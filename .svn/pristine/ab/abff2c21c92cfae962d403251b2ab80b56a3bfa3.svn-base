...
if (mysql_num_rows($ResultadoExisteLogin)==1)
{
	...
	//la pass introducida por el usuario no es correcta para ese login
	{
		...
		//del hiperenlace
		$error = new Error($Idioma['Error al introducir la password para ese login'],'../controllers/Login.php',$Idioma['Ir al login'],$Idioma);
...
else
{
	//se incluye la clase de vista de error V_Error.php
	include '../views/V_Error.php';
	//instanciamos para error de login no existente
	$error = new Error($Idioma['Error, no existe ese login'],'../controllers/Login.php',$Idioma['Ir al login'],$Idioma);
	//invocar el metodo Display()
	$error->Display();
}
